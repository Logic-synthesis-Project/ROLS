# ROLS
## [ROLS:Agent-Coordinated Reinforcement Learning for Optimization Fault Detection in Logic Synthesis]
...
### Our Works
Logic synthesis applies diverse optimization sequences to improve design quality, but such optimizations may introduce optimization faults. Detecting these faults is challenging due to the vast space of optimization flows. To address this, we proposes ROLS, an agent-coordinated reinforcement learning framework for optimization fault detection in logic synthesis. ROLS comprises three components: a Test Program Selector that selects structurally diverse Verilog programs via feature-based cosine sampling; an Optimization Selector that models each synthesis transformation as a learning agent jointly trained under an Advantage Actor–Critic (A2C) scheme to explore fault-inducing optimization sequences; and a Fault Verifier that performs formal equivalence checking to confirm detected faults. Experiments on Vivado and Yosys show that ROLS identifies 32 previously unknown optimization faults, achieving up to 31× improvement in detection efficiency over baseline fuzzing methods. ROLS received recognition from the Vivado community for its significant contributions to tool improvement.

### RQ. Effectiveness of Timing Features

To validate the effectiveness of the newly introduced timing features, we conducted an ablation study to compare the performance of ROLS with and without timing features. We follow the setup in Section III.A to construct feature vectors and use the same 1,000 diverse test programs in both configurations.

The experiments were conducted over a two-week testing period using ROLS to evaluate the effectiveness of timing features in fault detection. Both Vivado and Yosys synthesis tools were tested under identical hardware environments and experimental conditions. Two feature configurations were compared: ROLS without timing features and ROLS with timing features included. In the former configuration, the feature vectors consisted of five traditional categories: data processing and operations, data flow control, structuring and modularization, control flow and logic, and abstraction and reuse. In the latter configuration, timing features (e.g., the number of clock signals, flip-flops, edge-triggered logic, and non-blocking assignments) were added to capture the temporal characteristics of hardware circuits. For both configurations, ROLS employed the same multi-agent reinforcement learning framework to dynamically generate optimization sequences and applied them to the same set of 1,000 diverse test programs. The primary evaluation metric was the number of detected faults recorded under each configuration over the testing period.

As shown in **Table 1**, the inclusion of timing features improved the fault detection capabilities of ROLS. On Vivado, ROLS with timing features detected 16 faults, compared to 10 faults without timing features. Similarly, on Yosys, 5 faults were detected with timing features, while only 3 faults were detected without them.
